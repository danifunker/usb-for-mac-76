...
int main(void)
{
	...
	if(result != noErr)
		result = KDRV_FindPCICardByName(&sonnetRegEntryID, NEC_USB2_GESTALT_NAME);
	if(result != noErr)
		result = KDRV_FindPCICardByName(&sonnetRegEntryID, KEYLARGO_USB_GESTALT_NAME);
	if(result != noErr)
		result = KDRV_FindPCICardByName(&sonnetRegEntryID, ORANGE_USB_GESTALT_NAME);
